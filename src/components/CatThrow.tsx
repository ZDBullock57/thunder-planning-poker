...
const FlyingCat = ({ targetRef, onComplete }: FlyingCatProps) => {
  ...
  const [catColor] = useState(() => CAT_COLORS[Math.floor(Math.random() * CAT_COLORS.length)])
  ...
  useEffect(() => {
    ...
    switch (edge) {
      case 0: startX = Math.random() * w; startY = -60; break
      case 1: startX = Math.random() * w; startY = h + 60; break
      case 2: startX = -60; startY = Math.random() * h; break
      default: startX = w + 60; startY = Math.random() * h; break
    }
...
export const CatThrowManager = ({ events, getTargetRef, onEventComplete }: CatThrowManagerProps) => {
  return (
    <>
      {events.map((event) => {
